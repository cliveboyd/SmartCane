/*
 *		Source Code
 *		Extract from....  https://github.com/Martinsbl/mpu9150-example-nrf51/blob/master/mpu9150.c
 */

#include <stdbool.h>
#include <stdint.h>
#include "nrf_drv_twi.h"
#include "MPU9150_nRF51.h"
#include "MPU9150_register_map_nRF51.h"
...
static uint32_t mpu9150_write_burst(uint8_t reg, uint8_t const * p_data, uint32_t length) {
    uint32_t err_code;
...
uint32_t mpu9150_read_registers(uint8_t reg, uint8_t * p_data, uint32_t length) {
    ...
    err_code = nrf_drv_twi_rx(m_twi_instance, MPU9150_ADDRESS, p_data, length, false);
    if(err_code != NRF_SUCCESS) return err_code;
